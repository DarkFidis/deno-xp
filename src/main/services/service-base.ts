import { Serviceable, ServiceStateable } from '../types/service.ts'
import { InitBase } from './init-base.ts'
...
abstract class ServiceBase<T> extends InitBase<T> implements Serviceable<T> {
	...
	protected constructor(name: string, log: any, defaultConfig?: T) {
	...
	public async stop(): Promise<boolean> {
		if (!this.state.started || this.state.stopping) {
			if (this.state.stopping) {
				this._log.warn(
					`service ${this.name} is already stopping : ignore`,
				)
			} else {
